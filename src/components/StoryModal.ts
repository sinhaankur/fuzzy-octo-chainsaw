...
export function openStoryModal(data: StoryData): void {
  ...
  modalEl.innerHTML = `
    <div class="story-modal">
      <button class="story-close-x" aria-label="${t('modals.story.close') || 'Close'}">
      ...
      <div class="story-modal-content">
        <div class="story-loading">
          ...
          <span>${t('modals.story.generating') || 'Generating story...'}</span>
        </div>
      </div>
      <div class="story-share-bar" style="display:none">
        <button class="story-share-btn story-save" title="${t('modals.story.save') || 'Save PNG'}">
          ...
          <span>${t('modals.story.save') || 'Save'}</span>
        </button>
        <button class="story-share-btn story-whatsapp" title="${t('modals.story.whatsapp') || 'WhatsApp'}">
          ...
          <span>${t('modals.story.whatsapp') || 'WhatsApp'}</span>
        </button>
        <button class="story-share-btn story-twitter" title="${t('modals.story.twitter') || 'X / Twitter'}">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
          <span>${t('modals.story.twitter') || 'X'}</span>
        </button>
        <button class="story-share-btn story-linkedin" title="${t('modals.story.linkedin') || 'LinkedIn'}">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          <span>${t('modals.story.linkedin') || 'LinkedIn'}</span>
        </button>
        <button class="story-share-btn story-copy" title="${t('modals.story.copyLink') || 'Copy Link'}">
          ...
          <span>${t('modals.story.copyLink') || 'Link'}</span>
        </button>
      </div>
    </div>
  `;
  ...
  requestAnimationFrame(async () => {
    ...
    try {
      ...
      if (content) content.innerHTML = `<div class="story-error">${t('modals.story.error') || 'Failed to generate story.'}</div>`;
    }
  });
}
...
function downloadStory(): void {
  ...
  a.click();
  flashButton('.story-save', t('modals.story.saved') || 'Saved!', t('modals.story.save') || 'Save');
}
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  try {
    ...
    flashButton('.story-whatsapp', t('modals.story.copied') || 'Copied!', t('modals.story.whatsapp') || 'WhatsApp');
  } catch {
    downloadStory();
    flashButton('.story-whatsapp', t('modals.story.saved') || 'Saved!', t('modals.story.whatsapp') || 'WhatsApp');
  }
...
async function shareTwitter(data: StoryData): Promise<void> {
  ...
  flashButton('.story-twitter', t('modals.story.opening') || 'Opening...', t('modals.story.twitter') || 'X');
}

async function shareLinkedIn(data: StoryData): Promise<void> {
  const urls = getShareUrls(data);
  window.open(urls.linkedin, '_blank');
  flashButton('.story-linkedin', t('modals.story.opening') || 'Opening...', t('modals.story.linkedin') || 'LinkedIn');
}

async function copyDeepLink(data: StoryData): Promise<void> {
  const link = generateStoryDeepLink(data.countryCode);
  await navigator.clipboard.writeText(link);
  flashButton('.story-copy', t('modals.story.copied') || 'Copied!', t('modals.story.copyLink') || 'Link');
}
