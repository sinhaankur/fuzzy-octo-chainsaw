...
describe('resilience ranking contracts', () => {
  ...
  it('returns the cached ranking payload unchanged when the ranking cache already exists', async () => {
    ...
    redis.set('resilience:ranking:v3', JSON.stringify(cached));

    const response = await getResilienceRanking({ request: new Request('https://example.com') } as never, {});

    assert.deepEqual(response, cached);
    assert.equal(redis.has('resilience:score:v3:YE'), false, 'cache hit must not trigger score warmup');
  });
  ...
  it('returns all-greyed-out cached payload without rewarming (items=[], greyedOut non-empty)', async () => {
    ...
    assert.deepEqual(response, cached);
    assert.equal(redis.has('resilience:score:v3:SS'), false, 'all-greyed-out cache hit must not trigger score warmup');
  });
  ...
  it('warms missing scores synchronously and returns complete ranking on first call', async () => {
    ...
    redis.set('resilience:score:v3:NO', JSON.stringify({
      countryCode: 'NO',
      overallScore: 82,
      level: 'high',
    ...
    redis.set('resilience:score:v3:US', JSON.stringify({
      countryCode: 'US',
      overallScore: 61,
      level: 'medium',
      domains: domainWithCoverage,
    ...
    assert.ok(redis.has('resilience:score:v3:YE'), 'missing country should be warmed during first call');
    assert.ok(response.items.every((item) => item.overallScore >= 0), 'ranked items should all have computed scores');
    assert.ok(redis.has('resilience:ranking:v3'), 'fully scored ranking should be cached');
  });
});
